...
public class EfCodeFirstStudentRepository : IStudentRepository
{
    private readonly IDbContextFactory<GamifiedLearningCodeFirstContext> _contextFactory;

    public EfCodeFirstStudentRepository(IDbContextFactory<GamifiedLearningCodeFirstContext> contextFactory)
    {
    ...
    public async Task<IReadOnlyList<Student>> LoadAsync(CancellationToken cancellationToken = default)
    {
        ...
        return students.Select(StudentDomainMapper.ToDomain).ToList();
    }
    ...
    public async Task SaveGraphAsync(IEnumerable<Student> students, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var student in payload)
        {
            ...
            if (entity == null)
            {
                entity = new CodeFirst.Entities.StudentEntity
                {
                    Id = student.Id
                };
            ...
            StudentDomainMapper.ApplyChanges(entity, student);
        }
        ...
        foreach (var student in trackedStudents)
        {
            ...
            else
            {
                ...
                foreach (var assignmentToRemove in assignmentsToRemove)
                {
                    ...
                    {
                        context.Set<CodeFirst.Entities.AssignmentEntity>().Remove(assignmentToRemove);
                    }
